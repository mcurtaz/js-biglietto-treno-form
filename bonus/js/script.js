...
var bigliettoCompilato = false;

var d = new Date();

// creo una variabile che contiene la stringa della data odierna

var oggi = d.getDate() + "/" + (d.getMonth() + 1) +"/" + d.getFullYear();

// stampo data e costo al kilometro

document.getElementById("oggi").innerHTML = oggi;
...
// applico classe show per effetto transizione sull'opacità
  ...
  // cambio la variabile per sapere se il biglietto è stato Compilato

  bigliettoCompilato = true;
...
document.getElementById("btnAnnulla").addEventListener("click", function(){
  ...
  bigliettoCompilato = false;
});

document.getElementById("btnStampa").addEventListener("click", function(){
  if (bigliettoCompilato == true){
    alert("Mi dispiace i web developer di trenitalia non sanno ancora implementare questa funzione. Riprova ad ottobre.");
  } else{
    alert("Prima compila il form e premi Crea Biglietto");
  }
});
